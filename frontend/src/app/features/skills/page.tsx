...
import { Puzzle, Shield, Code, ChevronRight } from 'lucide-react'
  ...
  const handleLaunch = async () => {
    ...
    try {
      ...
      const created = await api.workflows.create({
        name: parsed.workflow_name || 'Autonomous Skills Injector',
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-6 pt-32 pb-24 space-y-12 z-10">
        ...
        <div className="flex items-center gap-4 border-b border-[#EAEAEA]/40 pb-6">
          ...
          <div>
            ...
            <h1 className="text-3xl font-serif font-bold text-[#111111] tracking-tight">Agent Skills & Integrations</h1>
          </div>
        </div>

        <p className="text-xs text-zinc-500 leading-relaxed font-semibold">
          Package your professional business expertise. Equip digital agents with custom API keys, execution schedules, and custom system prompt scripts autonomously.
        </p>
        ...
        <div className="bg-white border border-[#EAEAEA] rounded-3xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[11px] text-zinc-600 font-bold">
            <div className="flex items-center gap-2">• System prompts customization & injections</div>
            <div className="flex items-center gap-2">• API credential key vaults encryption</div>
            <div className="flex items-center gap-2">• Self-healing runtime workflow modules</div>
            <div className="flex items-center gap-2">• Connectors integrations with third-party tools</div>
          </div>
        </div>
        ...
        <div className="bg-white border-2 border-black rounded-3xl p-8 text-center space-y-6">
          <div className="space-y-2">
            <h3 className="text-xl font-serif font-bold text-[#111111]">Deploy Secure Compliance Skill</h3>
            <p className="text-xs text-zinc-500 max-w-xs mx-auto leading-relaxed">
              Let the agent customize secure workflow integrations and run SOC2 regulatory contract compliance checks.
            </p>
          </div>
          ...
          >
            {loading ? 'Deploying Pipeline...' : '🚀 Launch Skills Injector Swarm'}
          </button>
        </div>
