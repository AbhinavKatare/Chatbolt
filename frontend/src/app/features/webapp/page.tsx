...
import { Globe, Shield, Code, ChevronRight, Zap } from 'lucide-react'
import { getSession, api } from '@/lib/api'

export default function WebAppFeaturePage() {
  const router = useRouter()
  const [loading, setLoading] = useState(false)

  const handleLaunch = async () => {
    setLoading(true)
    const promptText = 'Build a premium responsive Next.js portfolio website with HSL tailored dark mode, Outfit typography, and floating micro-animations.'
    try {
      ...
      const created = await api.workflows.create({
        name: parsed.workflow_name || 'Autonomous WebApp Engine',
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-6 pt-32 pb-24 space-y-12 z-10">
        ...
        <div className="flex items-center gap-4 border-b border-[#EAEAEA]/40 pb-6">
          ...
          <div>
            ...
            <h1 className="text-3xl font-serif font-bold text-[#111111] tracking-tight">Full-Stack AI Web App Engine</h1>
          </div>
        </div>

        <p className="text-xs text-zinc-500 leading-relaxed font-semibold">
          Scaffold complete Next.js projects structured with App Router and Tailwind CSS dynamically. The web app agent writes components, configures routing paths, and deploys builds automatically.
        </p>
        ...
        <div className="bg-white border border-[#EAEAEA] rounded-3xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[11px] text-zinc-600 font-bold">
            <div className="flex items-center gap-2">• Next.js App Router static/dynamic scaffolds</div>
            <div className="flex items-center gap-2">• Tailwind CSS color palettes & Outfits configuration</div>
            <div className="flex items-center gap-2">• Express/Node.js backend API routing</div>
            <div className="flex items-center gap-2">• Direct Vercel / Netlify build deployments</div>
          </div>
        </div>
        ...
        <div className="bg-white border-2 border-black rounded-3xl p-8 text-center space-y-6">
          <div className="space-y-2">
            <h3 className="text-xl font-serif font-bold text-[#111111]">Deploy Active Portfolio Template</h3>
            <p className="text-xs text-zinc-500 max-w-xs mx-auto leading-relaxed">
              Let the agent construct and publish a premium responsive portfolio website with custom dark-mode toggles now.
            </p>
          </div>
          ...
          >
            {loading ? 'Deploying Pipeline...' : '🚀 Launch WebApp Engine Swarm'}
          </button>
        </div>
