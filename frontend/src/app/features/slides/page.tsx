...
import { FileText, Shield, Code, ChevronRight } from 'lucide-react'
...
export default function SlidesFeaturePage() {
  ...
  const handleLaunch = async () => {
    setLoading(true)
    const promptText = 'Create a slide presentation explaining Chatbolt business capabilities, multi-agent fleet operations, and ROI for enterprise sales.'
    try {
      ...
      const created = await api.workflows.create({
        name: parsed.workflow_name || 'Autonomous Slides Engine',
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-6 pt-32 pb-24 space-y-12 z-10">
        ...
        <div className="flex items-center gap-4 border-b border-[#EAEAEA]/40 pb-6">
          ...
          <div>
            ...
            <h1 className="text-3xl font-serif font-bold text-[#111111] tracking-tight">AI Slides & Presentation Creator</h1>
          </div>
        </div>

        <p className="text-xs text-zinc-500 leading-relaxed font-semibold">
          Transform raw textual documents, briefs, or business models into beautifully outlined, high-impact slides decks dynamically using PPTXGenJS integrations.
        </p>
        ...
        <div className="bg-white border border-[#EAEAEA] rounded-3xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[11px] text-zinc-600 font-bold">
            <div className="flex items-center gap-2">• Slide structural layout outlines mapping</div>
            <div className="flex items-center gap-2">• Harmonious color grids & high-contrast graphics</div>
            <div className="flex items-center gap-2">• Exporters support for raw PPTX / PDF packages</div>
            <div className="flex items-center gap-2">• Integrated speaker notations and slide-transitions</div>
          </div>
        </div>
        ...
        <div className="bg-white border-2 border-black rounded-3xl p-8 text-center space-y-6">
          <div className="space-y-2">
            <h3 className="text-xl font-serif font-bold text-[#111111]">Deploy B2B Capability Slides</h3>
            <p className="text-xs text-zinc-500 max-w-xs mx-auto leading-relaxed">
              Let the agent generate a complete presentation detailing Chatbolt ROI capabilities and multi-agent systems.
            </p>
          </div>
          ...
          >
            {loading ? 'Deploying Pipeline...' : '🚀 Launch Slides Creator Swarm'}
          </button>
        </div>
